    ...
    for(int i = 1; i < argc; ++i) {
        if(argv[i][0] == '-') {
            if(strlen(argv[i]) > 1) {
                ...
                LYSLG_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                    << " val=" << argv[i];
                return false;
            }
        } else {
...
void Env::printArgs() {
    for(auto x:m_args) {
        std::cout <<  "key="<< x.first << "values" << x.second << std::endl;
    }
}
...
//std::setw(5) 会设置输出流的字段宽度为 5 ,仅影响接下来的输出
void Env::printHelp() {
    RWMutexType::ReadLock lock(m_mutex);
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const {
    ...
    static lyslg::ConfigVar<std::string>::ptr g_server_work_path =
        lyslg::Config::Lookup<std::string>("server.work_path");
