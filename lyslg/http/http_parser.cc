...
static lyslg::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    lyslg::Config::Lookup("http.request.buffer_size"
                ,(uint64_t)(4 * 1024), "http request buffer size");

static lyslg::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    lyslg::Config::Lookup("http.request.max_body_size"
                ,(uint64_t)(64 * 1024 * 1024), "http request max body size");

static lyslg::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    lyslg::Config::Lookup("http.response.buffer_size"
                ,(uint64_t)(4 * 1024), "http response buffer size");

static lyslg::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    lyslg::Config::Lookup("http.response.max_body_size"
...
void on_request_method(void *data, const char *at, size_t length) {
    ...
    if(m == HttpMethod::INVALID_METHOD) {
        LYSLG_LOG_WARN(g_logger) << "invalid http request method: "
...
void on_request_fragment(void *data, const char *at, size_t length) {
    //LYSLG_LOG_INFO(g_logger) << "on_request_fragment:" << std::string(at, length);
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
