...
public:
    ...
    void operator()(Module* module) {
        ...
        if(rt) {
            LYSLG_LOG_ERROR(g_logger) << "dlclose handle fail handle="
                << m_handle << " name=" << name
                << " version=" << version
                << " path=" << path
                << " error=" << dlerror();
        } else {
            LYSLG_LOG_INFO(g_logger) << "destory module=" << name
                << " version=" << version
                << " path=" << path
                << " handle=" << m_handle
                << " success";
...
Module::ptr Library::GetModule(const std::string& path) {
    ...
    if(!handle) {
        LYSLG_LOG_ERROR(g_logger) << "cannot load library path="
            << path << " error=" << dlerror();
        return nullptr;
    }
    ...
    if(!create) {
        LYSLG_LOG_ERROR(g_logger) << "cannot load symbol CreateModule in "
            << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    if(!destory) {
        LYSLG_LOG_ERROR(g_logger) << "cannot load symbol DestoryModule in "
            << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    LYSLG_LOG_INFO(g_logger) << "load module name=" << module->getName()
        << " version=" << module->getVersion()
        << " path=" << module->getFilename()
        << " success";
    Config::LoadFromConfDir(lyslg::EnvMgr::GetInstance()->getConfigPath(), true);
    return module;
}
