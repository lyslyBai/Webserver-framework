...
#define XX(type,len,write_fun,read_fun,base_len) {\
    ...
    lyslg::ByteArray::ptr ba(new lyslg::ByteArray(base_len)); \
    for(auto& i : vec) { \
        ba->write_fun(i); \
    } \
    ba->setPosition(0); \
    ...
    LYSLG_ASSERT(ba->getReadSize() == 0); \
    LYSLG_LOG_INFO(g_logger) << #write_fun "/" #read_fun \
        " (" #type ") len=" << len \
        << " base_len=" << base_len \
        << " size=" << ba->getSize(); \
}
...
#define XX(type,len,write_fun,read_fun,base_len) {\
    ...
    LYSLG_LOG_INFO(g_logger) << #write_fun "/" #read_fun \
        ...
        << " size=" << ba->getSize(); \
    ba->setPosition(0); \
    LYSLG_ASSERT((ba->writeToFile("../tmp/" #type "_" #len "_" #read_fun ".dat"))); \
    lyslg::ByteArray::ptr ba2(new lyslg::ByteArray(base_len * 2)); \
    LYSLG_ASSERT((ba2->readFromFile("../tmp/" #type "_" #len "_" #read_fun ".dat"))); \
    ba2->setPosition(0); \
    LYSLG_ASSERT(ba->toString() == ba2->toString()); \
    LYSLG_ASSERT(ba->getPosition() == 0); \
    LYSLG_ASSERT(ba2->getPosition() == 0); \
}    
